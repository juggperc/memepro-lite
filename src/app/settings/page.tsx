            ...
            <main className="flex-1 p-6">
                <div className="max-w-2xl mx-auto space-y-8">
                    ...
                    <section className="space-y-6">
                        ...
                        {/* Slippage */}
                        <div className="bg-[#0a0a0a] border border-[#1a1a1a] p-4 rounded-sm space-y-4 shadow-xl">
                            <div>
                                <label className="text-xs text-[#888] block mb-2 font-medium">Default Slippage</label>
                                <div className="flex gap-2">
                                    {[100, 300, 500, 1000].map((bps) => (
                                        <button
                                            key={bps}
                                            onClick={() => setDefaultSlippage(bps)}
                                            className={`flex-1 py-2 text-xs border transition-all ${defaultSlippage === bps
                            ...
                            {/* Priority Fee */}
                            <div>
                                <label className="text-xs text-[#888] block mb-2 font-medium">Priority Fee Mode</label>
                                <div className="flex gap-2">
                                    {(['auto', 'low', 'medium', 'high'] as const).map((mode) => (
                                        <button
                                            key={mode}
                                            onClick={() => setPriorityFeeMode(mode)}
                                            className={`flex-1 py-2 text-xs border capitalize transition-all ${priorityFeeMode === mode
                                                ? 'bg-white text-black border-white'
                                                : 'bg-transparent text-[#666] border-[#333] hover:text-white hover:border-[#666]'
                                                }`}
                                        >
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-[#1a1a1a]">
                                <div className="flex flex-col">
                                    <span className="text-sm text-white font-medium">MEV Protection (Jito)</span>
                    ...
                    <div className="text-center pt-8 border-t border-[#1a1a1a]">
                        <div className="text-[10px] text-[#444] font-mono tracking-widest uppercase">memepro.lite v0.1.0</div>
                    </div>
                </div>
            </main>
        </div>
